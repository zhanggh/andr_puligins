...
public class UpdateManager {
	static DingLog log = new DingLog(UpdateManager.class);
	...
	public void checkAppUpdate(Context context, final boolean isShowMsg){
		...
		final Handler handler = new Handler(){
			public void handleMessage(Message msg) {
				...
				//显示检测结果
				if(msg.what == 1){
					appInfo = (AppInfoModel)msg.obj;
					if(appInfo != null){
						if(curVersionCode < Integer.parseInt(appInfo.getVersion())){
							apkUrl = appInfo.getPath();
							if(FuncUtil.isEmpty(appInfo.getInfo())){
								updateMsg="发现最新版本app，点击确认进行更新";	
							}else{
								updateMsg = appInfo.getInfo();
							}
							showNoticeDialog();
						}else if(isShowMsg){
		...
		new Thread(){
			public void run() {
				...
				try {	
					...
					rsp = ComApp.getInstance().getApi().sysAppupdate(String.valueOf(curVersionCode));
					msg.what = 1;
					if(rsp!=null)
						msg.obj = rsp.getAppInfo();
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private void showNoticeDialog(){
		...
		builder.setPositiveButton("立即更新", new OnClickListener() {			
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				XHSDKUtil.addXHBehavior(ComApp.getInstance(),user, XHConstants.XHTOPIC_INSTALL,user+" is updating a new version of app");
				showDownloadDialog();
			}
		});
