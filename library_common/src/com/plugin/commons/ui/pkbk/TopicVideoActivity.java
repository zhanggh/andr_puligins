...
public class TopicVideoActivity extends BaseActivity{
	...
	{		
		...
		im_tkphoto.setBackgroundDrawable(this.getResources().getDrawable(ComApp.getInstance().appStyle.boke_btn_selector));
		lv_news = (PullToRefreshListView) this.findViewById(R.id.lv_news);
		mAdapter = new ComVideoListAdapter(this,dataList);//ComPhotListAdapter(this, dataList);
		lv_news.setAdapter(mAdapter);
		lv_news.setOnRefreshListener(new OnRefreshListener<ListView>() {
			@Override
			public void onRefresh(PullToRefreshBase<ListView> refreshView) {
				String label = DateUtils.formatDateTime(TopicVideoActivity.this, System.currentTimeMillis(),
...
//				Toast.makeText(TopicPhotoActivity.this, "已无更多加载", Toast.LENGTH_SHORT).show();
		...
		lv_news.setOnItemClickListener(new OnItemClickListener(){
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
					long arg3) {
				XHSDKUtil.addXHBehavior(TopicVideoActivity.this, mNewType.getParentid()+"_"+dataList.get(arg2-1).getId(), XHConstants.XHTOPIC_ARTICAL_CLICK, dataList.get(arg2-1).getId());
				ComUtil.goNewsDetail(TopicVideoActivity.this, dataList.get(arg2-1), mNewType);
		...
		im_tkphoto.setOnClickListener(new OnClickListener(
			...
			public void onClick(View v) {
				...
				cmmodel.setThemeid(mNews.getId());
				 Intent intent=new Intent(TopicVideoActivity.this, UploadVideoActivity.class);
	...
	private void doRefresh(final boolean isInit,final boolean isRefresh)
	{
		...
		sCallBack=new SituoAjaxCallBackImp<NewsInfoModel,NewsService>(findViewById(R.id.ll_root),pageStart,this.dataList,isInit, isRefresh,
			...
			public void afterService(Map<String,NewsInfoModel> response ,List<NewsInfoModel> dataList,int pageStart) {
				TopicVideoActivity.this.pageStart=pageStart;
				TopicVideoActivity.this.dataList=dataList;
			}
		};
