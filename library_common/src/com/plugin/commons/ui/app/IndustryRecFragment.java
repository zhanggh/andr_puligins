...
public class IndustryRecFragment extends BaseFragment {

	private List<AppInfoModel> industry_app_list;//热门列表
	protected TextView tv_modelTile;
	protected LinearLayout ll_datalist;
	...
	protected  void initViews(View view){
		...
		int num=3;
		if(this.getIndustry_app_list().size()<3){
			num=this.getIndustry_app_list().size();
		}
		for(int a=0;a<num+1;a++){
			final AppInfoModel appInfo=this.getIndustry_app_list().get(a);
...
//				img.setBackgroundResource(R.drawable.pk001);
				...
				public void onClick(View v) {
					Intent intent;
					 if(v.getId()==3999){
						 ...
						 intent.putExtra(CoreContants.PARAMS_MSG_ID,CoreContants.REQUEST_APP_INDUSTRY);
	...
	public List<AppInfoModel> getIndustry_app_list() {
		if(industry_app_list==null){
			industry_app_list=new ArrayList<AppInfoModel>();
		}
		return industry_app_list;
	}
	public void setIndustry_app_list(List<AppInfoModel> industry_app_list) {
		this.industry_app_list = industry_app_list;
	}
}
